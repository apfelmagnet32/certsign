...
def _find_uno_python():
    for candidate in _UNO_PYTHON_CANDIDATES:
        ...
        if result.returncode == 0:
            return candidate
    raise SigningError(
        "Kein Python mit UNO-Anbindung gefunden. Installiere 'python3-uno' "
        "(Debian/Ubuntu) bzw. die LibreOffice-SDK-Pakete deiner Distribution."
    )
...
def _wait_for_port(host, port, timeout=30):
    ...
    while time.time() < deadline:
        try:
            ...
            time.sleep(0.5)
    raise SigningError("LibreOffice ist nach dem Start nicht auf dem UNO-Socket erreichbar geworden.")
...
def sign_docx(pfx_path, password, infile, outfile):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            if result.returncode != 0:
                raise SigningError(
                    "LibreOffice-Signierung fehlgeschlagen: "
